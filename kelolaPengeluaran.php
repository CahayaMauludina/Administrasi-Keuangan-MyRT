...
<body>
    ...
    <div class="container mt-4 border rounded-4 shadow">
        <div class="row">
            <div class="col">
                ...
                <div class="p-3">
                    <table class="table table-striped text-center" id="tablePengeluaran">
                        <thead>
    ...
    <script>
      document.addEventListener("DOMContentLoaded", async function(){
				...
				const dataPengeluaran = await getDataPengeluaran();
				...
				function renderDataPengeluaran(data) {
					 const table = document.querySelector('#tablePengeluaran tbody');
					 table.innerHTML = '';
					 let number = 0
					 data.forEach(item => {
						 ...
						 tr.innerHTML = `
							 ...
							 <td><div class="d-flex">
									...
									<button type="button" class="btn btn-danger me-3 btnDelete" data-id="${item.id_pengeluaran}">
											Delete
									</button>
							</div></td>`;
            ...
            // JS untuk isi data modal edit
				$('#modalEdit').on('show.bs.modal', function(e) {
					...
					const data = dataPengeluaran.find(item => item.id_pengeluaran == id);
				...
				$('#formTambah').submit(function(event) {
						...
						$.ajax({
								type: 'POST',
								url: 'http://localhost:3000/tambah-pengeluaran',
								headers: {
				...
				$('#formEditPengeluaran').submit(function (event) {
					event.preventDefault(); // Hindari pengiriman formulir secara sinkron
					...
					$.ajax({
								type: 'POST',
								url: 'http://localhost:3000/edit-pengeluaran/' + user,
								headers: {
									'Content-Type': 'application/json'
								},
								data: JSON.stringify({
				...
				$('#tablePengeluaran').on('click', '.btnDelete', function (e) {
					...
					.then((willDelete) => {
						if (willDelete) {
							$.ajax({
								type: 'POST',
								url: 'http://localhost:3000/hapus-pengeluaran',
								headers: {
									'Content-Type': 'application/json'
								},
								data: JSON.stringify({
